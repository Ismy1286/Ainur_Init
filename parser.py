...
start = datetime.now()   # записываем время начало работы
...
def get_links():
    ...
    tds = soup.find('table', class_="table").find_all('td')  # получаем элементы таблицы
    links = []
...
def get_page_data():  #  вызываем функцию для получения данных с каждой страницы
    ...
    for link in links:
        ...
        try:
            ...
            try:
                phone = soup.find('p', class_='mb-10').text.strip()  #  пробуем получить тел
                # print(phone)
            except Exception as ex:
                print(ex)
                phone = 'Нет данных'
            ...
            try:
                bio = soup.find('div', id='biography').text.strip().replace('NBSP', '')  # пробуем получить био
                # print(len(bio))
            except Exception as ex:
                print(ex)
